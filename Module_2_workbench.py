...
# ── Experiment runner (threaded, live elapsed time) ───────────────────────────
def run_builder(cfg, df):
    ...
    def _work():
        try:
            ...
            coverage = builder.evaluate_final_coverage(df) if segments else []
            out.put(("done", segments, coverage, builder.stop_reason, None))
        except Exception as exc:
            out.put(("error", None, None, None, str(exc)))
    ...
    try:
        with st.status("Running experiment…", expanded=True) as status:
            while True:
                try:
                    kind, segments, coverage, stop_reason, err = out.get(timeout=0.5)
                    break
                except queue.Empty:
                    status.write(
                        f"Elapsed {fmt_duration(time.time() - t0)} — "
                        "scanning residual populations…"
                    )
            if kind == "error":
                status.update(label="Extraction failed", state="error", expanded=True)
            else:
                status.update(
                    label=f"Extraction complete — {len(segments)} segment(s)",
                    state="complete",
                    expanded=False,
                )
    except AttributeError:
        with st.spinner("Running experiment…"):
            kind, segments, coverage, stop_reason, err = out.get()
    if kind == "error":
