...
A Streamlit no-code UI on top of the **RapidSegment** PyPI library (`rapidsegment==1.2.2.post1`, class `StrategicSegmentBuilder`). Spec lives in `UI_MVP.md` (converted from Solara → Streamlit). 6 modules; 1–4 done, 5–6 pending.
...
- ⬜ M5 Leaderboard — read suite_data.db experiments; clone-to-workbench via apply_config; sparklines; row actions
- ⬜ M6 Arena — KPI face-off, param diff (only differing keys), segment comparison, SQL diff
